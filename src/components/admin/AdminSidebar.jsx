...
const AdminSidebar = () => {
  ...
  const isActive = (url) => url === '/admin' ? location.pathname === '/admin' : location.pathname.startsWith(url);
  ...
  return (
    <Sidebar collapsible="icon" className="border-r border-sidebar-border">
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel className="text-xs uppercase tracking-wider text-sidebar-foreground/50">Módulos</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {menuItems.map((item) => (
