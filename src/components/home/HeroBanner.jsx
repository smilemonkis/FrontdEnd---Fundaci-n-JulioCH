...
const FALLBACK_SLIDES = [
  { imagenUrl: heroBanner1, titulo: 'Transformamos el Suroeste',       subtitulo: 'Formación, empleo y cultura para nuestra comunidad',       ctaTexto: 'Conoce más',        ctaLink: '/quienes-somos' },
  { imagenUrl: heroBanner2, titulo: 'Educación que Transforma',         subtitulo: 'Abrimos convocatorias para el desarrollo de tu talento',    ctaTexto: 'Ver convocatorias', ctaLink: '/convocatorias'  },
  { imagenUrl: heroBanner3, titulo: 'Comunidad y Cultura',              subtitulo: 'Celebramos la riqueza del Suroeste Antioqueño',             ctaTexto: 'Párchate',          ctaLink: '/parchate'       },
];
  ...
  return (
    <section className="relative w-full h-[60vh] md:h-[75vh] overflow-hidden">
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2">
        {slides.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)} className={`h-3 rounded-full transition-all ${i === current ? 'bg-primary w-8' : 'w-3 bg-background/50'}`} aria-label={`Ir a slide ${i + 1}`} />
        ))}
      </div>
    </section>
  );
