...
const consejo = ['Mercedes Hernández De Bedout', 'Rodrigo Puyo Vasco', 'David Duque Brumbaugh', 'Alfredo Tamayo Jaramillo', 'Luis Miguel De Bedout Hernández'];
const equipo = ['Ana María Ramirez Carmona', 'Marlly Alejandra Montaño Gaviria'];
    ...
    <main>
      ...
      <section className="section-container section-padding">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="font-body text-lg text-muted-foreground leading-relaxed mb-4">Somos una entidad privada sin ánimo de lucro que, inspirada en el legado de{' '}<strong className="text-foreground">don Julio C. Hernández Fernández</strong> y en el trabajo continuo de su hermano{' '}<strong className="text-foreground">Sergio Hernández</strong>, promueve el acceso a la educación como camino hacia el desarrollo humano, la equidad y la movilidad social.</p>
          <p className="font-body text-muted-foreground leading-relaxed mb-4">Nacimos del compromiso de una familia y de un territorio con la educación. Desde entonces, hemos enfocado nuestros esfuerzos en brindar{' '}<strong className="text-foreground">oportunidades reales de formación</strong>, especialmente a jóvenes y adultos del{' '}<strong className="text-foreground">Suroeste Antioqueño</strong>, que buscan construir un mejor futuro a través del conocimiento.</p>
          <p className="font-body text-muted-foreground leading-relaxed">Creemos en el poder de la unión entre aliados, instituciones y comunidad, porque solo trabajando juntos podemos abrir más puertas a quienes sueñan con estudiar y progresar. Por eso, cada uno de nuestros programas y proyectos está diseñado para{' '}<strong className="text-foreground">acompañar, fortalecer y transformar</strong>.</p>
        </div>
      </section>
      ...
      <section className="bg-primary/5">
        <div className="section-container py-16 md:py-24 space-y-20">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            ...
            <div className="rounded-2xl overflow-hidden shadow-lg"><img src={nosotrosMision} alt="Misión" className="w-full h-72 md:h-80 object-cover" loading="lazy" /></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
            <div className="order-2 md:order-1 rounded-2xl overflow-hidden shadow-lg"><img src={nosotrosVision} alt="Visión" className="w-full h-72 md:h-80 object-cover" loading="lazy" /></div>
            <div className="order-1 md:order-2">
              ...
              <p className="font-body text-muted-foreground leading-relaxed">Nos proyectamos como una institución coadyuvante en el proceso educativo de <strong className="text-foreground">Medellín y de Colombia</strong>, que trascenderá en el tiempo mediante la realización de obras de educación para fortalecer y mejorar la calidad de vida de las comunidades más necesitadas del país.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section-container section-padding">
        <div className="text-center mb-12">
          ...
          <p className="font-body text-muted-foreground max-w-2xl mx-auto">Parte de una convicción: <strong className="text-foreground">la educación es el camino más sólido hacia la transformación social</strong>. Basamos nuestro actuar en cuatro pilares:</p>
        </div>
      ...
      <section className="section-container section-padding">
        ...
        <div className="max-w-4xl mx-auto mb-12">
          <h3 className="font-heading text-xl font-bold text-foreground mb-6 text-center">Consejo de Dirección</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {consejo.map((nombre) => (
              <div key={nombre} className="text-center">
                <div className="w-20 h-20 rounded-full bg-primary/10 flex items-center justify-center mx-auto mb-3"><Users className="w-8 h-8 text-primary/60" /></div>
                <p className="font-body text-sm font-medium text-foreground leading-tight">{nombre}</p>
              </div>
            ))}
          </div>
        </div>
        <div className="max-w-4xl mx-auto">
          <h3 className="font-heading text-xl font-bold text-foreground mb-6 text-center">Equipo Administrativo</h3>
          <div className="flex justify-center gap-8 flex-wrap">
            {equipo.map((nombre) => (
              <div key={nombre} className="text-center">
                <div className="w-20 h-20 rounded-full bg-secondary/10 flex items-center justify-center mx-auto mb-3"><Users className="w-8 h-8 text-secondary/60" /></div>
                <p className="font-body text-sm font-medium text-foreground leading-tight">{nombre}</p>
              </div>
            ))}
          </div>
