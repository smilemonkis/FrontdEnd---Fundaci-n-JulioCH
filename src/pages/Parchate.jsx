        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {items.map(p => (
                <div key={p.id} className="bg-card rounded-xl overflow-hidden border border-border group flex flex-col shadow-sm hover:shadow-md transition-shadow">
                  {/* Imagen fija */}
                  ...
                  {/* Contenido flex-1 para igualar altura entre cards */}
                  <div className="p-5 flex flex-col flex-1">
                    {/* Badge tipo + ubicación — altura fija */}
                    <div className="flex items-center justify-between mb-2 shrink-0">
                      <span className="px-2 py-0.5 text-xs font-body font-semibold rounded-full bg-primary/10 text-primary line-clamp-1">{p.tipo}</span>
                      <span className="flex items-center gap-1 text-xs font-body text-muted-foreground">
                        <MapPin className="w-3 h-3 shrink-0" />{p.ubicacion}
                      </span>
                    </div>

                    {/* Título — siempre 2 líneas */}
                    <h3 className="font-heading text-lg font-semibold text-foreground mb-1 line-clamp-2 shrink-0" style={{minHeight:'3.5rem'}}>{p.titulo}</h3>

                    {/* Descripción — siempre 2 líneas */}
                    <p className="font-body text-sm text-muted-foreground mb-3 line-clamp-2 shrink-0" style={{minHeight:'2.5rem'}}>{p.descripcion}</p>

                    {/* Fecha — espacio fijo aunque no haya fecha */}
